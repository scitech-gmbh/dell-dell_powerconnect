...
describe "Integration test for powerconnect config" do
  ...
  provider_class = Puppet::Type.type(:powerconnect_backup).provider(:dell_powerconnect)
  ...
  #Load Add HBA file
  switch_config_yml =  YAML.load_file(integrationYML('dell_powerconnect', 'powerconnect_backup','switch_config.yml'))

  create_node = switch_config_yml['SwitchConfig1']
    
  let :switch_config do
    Puppet::Type.type(:powerconnect_backup).new(
  ...
  context 'when applying configuration changes' do 
    it "should apply new configuration" do 
      ...
      result = config_instance.run(switch_config[:url], switch_config[:config_type])
